...
import (
	...
	ncpapi "nexcoreproxy-panel/ncp/api"
	ncpinstall "nexcoreproxy-panel/ncp/install"
	"nexcoreproxy-panel/sub"
	"nexcoreproxy-panel/util/crypto"
	"nexcoreproxy-panel/util/sys"
	"nexcoreproxy-panel/web"
	"nexcoreproxy-panel/web/global"
...
func runWebServer() {
	...
	// Start NCP management API server
	startNCPAPI()
...
// startNCPAPI starts the NCP management API server alongside the panel.
func startNCPAPI() {
	err := database.InitDB(config.GetDBPath())
	if err != nil {
		return
	...
	// Read NCP API port from settings, default 54322
	var portStr string
	db.Raw("SELECT value FROM settings WHERE `key` = 'ncpAPIPort'").Scan(&portStr)
	port := 54322
	if p, err := fmt.Sscanf(portStr, "%d", &port); err != nil || p == 0 {
		port = 54322
	}

	apiServer := ncpapi.NewServer(db, port)
	apiServer.Start()
}

func main() {
	// os.Args[0] detection: if invoked as "ncp-agent" (via symlink), enter agent mode
	binName := filepath.Base(os.Args[0])
	if strings.HasPrefix(binName, "ncp-agent") {
		// Initialize DB for CLI commands
	...
	case "ncp":
		// NCP agent CLI commands: x-ui ncp <subcommand>
		_ = database.InitDB(config.GetDBPath())
		agent.Run(os.Args[2:])
	case "install":
		// Built-in install: x-ui install --port X --user Y --pass Z --gen-token
		ncpinstall.Run(os.Args[2:])
	default:
		...
		fmt.Println("    setting        set settings")
		fmt.Println("    ncp            NCP management commands (ncp-agent)")
		fmt.Println("    install        configure panel (port, user, pass, token)")
	}
}
